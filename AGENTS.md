...
- Редактор `.siq` пока базовый: нет спецтипов вопросов, полного переноса SiQuester, импорта всех возможных XML-конструкций и одновременного запуска картинки с медиа.
- Типы вопросов/ответов уже есть как базовые поля, но сложная сценарная логика SiQuester для ставок, секретов и выбора ответа ещё не перенесена.
